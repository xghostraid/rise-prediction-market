...
// Keep DB handle broadly typed (no row inference).
export type Db = Sql<Record<string, unknown>>;

export function connectDb(databaseUrl: string): Db {
...
export async function getCursor(db: Db, key: string): Promise<string | null> {
  const rows = await db<{ value: string }[]>`select value from cursor where key = ${key}`;
